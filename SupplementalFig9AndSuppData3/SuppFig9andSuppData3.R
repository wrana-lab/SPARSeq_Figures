...
alldata<-read.xlsx("filtered_data_for_paper_v26.xlsx")
#fix dates
alldata$Date.of.collection <- as.Date(alldata$Date.of.collection,
...
secondtab<-secondtab/nrow(alldata)*100
...
fourthtab<-fourthtab/nrow(alldata)*100
...
#write.xlsx(list_of_datasets, file = "supplData3_data-v26.xlsx")
#We also manually added canonical/non canonical labelling 

#load file - these are the manually compiled muttables from above.
varfile<-read.xlsx("v26data_aggregatedvariants.xlsx", sheet = "All Variants")
varfilewt<-read.xlsx("v26data_aggregatedvariants.xlsx", sheet = "WT Variants")
varfilealpha<-read.xlsx("v26data_aggregatedvariants.xlsx", sheet = "Alpha Variants")
varfilebetagamma<-read.xlsx("v26data_aggregatedvariants.xlsx", sheet = "BetaGamma Variants")
varfiledelta<-read.xlsx("v26data_aggregatedvariants.xlsx", sheet = "Delta Variants")
varfileomicron<-read.xlsx("v26data_aggregatedvariants.xlsx", sheet = "Omicron Variants")
varfileba2.75<-read.xlsx("v26data_aggregatedvariants.xlsx", sheet = "BA2.75 Variants")
varfileba23<-read.xlsx("v26data_aggregatedvariants.xlsx", sheet = "BA23 Variants")
varfileba45<-read.xlsx("v26data_aggregatedvariants.xlsx", sheet = "BA45 Variants")
varfilexbb<-read.xlsx("v26data_aggregatedvariants.xlsx", sheet = "XBB Variants")
varfilexbb.1.5<-read.xlsx("v26data_aggregatedvariants.xlsx", sheet = "XBB.1.5 Variants")
...
approvedrdrp <- c("A685D", "A685E", "T686N", "A685H", "A685V", "A699C", "A702Stop", "N705R", "N705C", "N713Stop",
                  "C697P", "C697S", "G712Stop", "I696L", "I715I", "L707F", "L708I", "L708V", "T701C",
                  "T701S", "T710R", "V700N", "V700H", "V700Y", "V704K", "V704Stop", "V693F", "D711W")
...
#write.xlsx(allvarpositioncounts, file="uniquePositions_withVariants_perVariant_v26.xlsx")
...
varbarplot4<-ggplot(allvarpositioncounts_agg[-which(allvarpositioncounts_agg$amplicon == "amplicon"),], 
                    aes(x=variant, y=Positions, fill = variant))  + facet_wrap(.~amplicon, ncol=3) +
  geom_col(colour="black", position='dodge2') + ggtitle("AA Positions per Amplicon with a Variant") + scale_fill_manual(values=colvec) +
  xlab("Variant") + ylab("Count") + theme_bw() +  scale_x_discrete(drop=FALSE) +
  theme(axis.title = element_text(size=10), legend.position = c("none"), plot.background = element_blank(),
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.text = element_text(size = 8),
...
pdf("barplotsofVarCounts_SupFig9_v26data.pdf", width = 5, height = 3)
varbarplot3
varbarplot
varbarplot2
varbarplot4
